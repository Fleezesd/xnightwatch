...
// 幂等性令牌已过期错误
func IsIdepotentTokenExpired(err error) bool {
	if err == nil {
		return false
	}
	e := errors.FromError(err)
	return e.Reason == ErrorReason_IdepotentTokenExpired.String() && e.Code == 400
}

// 幂等性令牌已过期错误
func ErrorIdepotentTokenExpired(format string, args ...interface{}) *errors.Error {
	return errors.New(400, ErrorReason_IdepotentTokenExpired.String(), fmt.Sprintf(format, args...))
}
