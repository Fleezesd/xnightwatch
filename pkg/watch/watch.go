...
import (
	...
	"github.com/fleezesd/xnightwatch/pkg/watch/logger/empty"
	"github.com/go-redsync/redsync/v4"
	"github.com/go-redsync/redsync/v4/redis/goredis/v9"
	"github.com/redis/go-redis/v9"
	"github.com/robfig/cron/v3"

	stringsutil "github.com/fleezesd/xnightwatch/pkg/util/strings"
)
...
func NewWatch(opts *Options, client *redis.Client, withOptions ...Option) (*Watch, error) {
	...
	nw.runner = cron.New(
		...
		// Prevent multiple instances of the same task from running at the same time
	...
	// for all instances wanting the same loc
	...
	go func() {
		...
		for {
			...
			if ok, err := nw.locker.ExtendContext(ctx); !ok || err != nil {
				nw.logger.Error(err, "Failed to extend lock", "err", err, "status", ok)
			}
		}
	}()
