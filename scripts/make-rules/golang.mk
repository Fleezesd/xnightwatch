...
ifeq ($(X_ROOT),)
	$(error the variable X_ROOT must be set prior to including golang.mk)
endif


VERSION_PACKAGE := github.com/fleezesd/xnightwatch/pkg/version
...
CPUS := $(shell /bin/bash $(X_ROOT)/scripts/read_cpus_available.sh)
...
ifeq (${COMMANDS},)
  $(error Could not determine COMMANDS, set X_ROOT or run in source dir)
endif
ifeq (${BINS},)
  $(error Could not determine BINS, set X_ROOT or run in source dir)
endif

EXCLUDE_TESTS=github.com/fleezesd/xnightwatch/pkg/db,manifests
...
go.build.%: ## Build specified applications with platform, os and arch.
	...
	#@X_GIT_VERSION=$(VERSION) $(SCRIPTS_DIR)/build.sh $(COMMAND) $(PLATFORM)
	@if grep -q "func main()" $(X_ROOT)/cmd/$(COMMAND)/*.go &>/dev/null; then \
