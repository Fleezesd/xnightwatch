...
SERVICES ?= $(filter-out tools,$(foreach service,$(wildcard ${X_ROOT}/cmd/*),$(notdir ${service})))
...
gen.helm-docs: tools.verify.helm-docs ## Generate documentation for helm charts.
	...
	@helm-docs --log-level=warning --chart-search-root $(X_ROOT)
...
gen.protoc: ## Generate go source files from protobuf files.
	@protoc \
		...
		--openapi_out=fq_schema_naming=true,default_response=false:$(X_ROOT)/api/openapi \
		--openapiv2_out=$(X_ROOT)/api/openapi \
...
gen.systemd.%: ## Generate specified systemd unit file.
	...
	@$(SCRIPTS_DIR)/gen-service-config.sh $(SERVICE) $(ONEX_ENV_FILE) \
		$(X_ROOT)/configs/onex.systemd.tmpl.service $(GENERATED_SERVICE_DIR)
ifeq ($(V),1)
...
gen.appconfig.%: ## Generate specified application configuration file.
	...
	@$(SCRIPTS_DIR)/gen-service-config.sh $(SERVICE) $(ONEX_ENV_FILE) \
		$(X_ROOT)/configs/appconfig/$(SERVICE).config.tmpl.yaml $(GENERATED_SERVICE_DIR)
ifeq ($(V),1)
