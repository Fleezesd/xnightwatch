...
ifeq ($(origin X_ROOT),undefined)
X_ROOT :=$(shell dirname $(realpath $(firstword $(MAKEFILE_LIST))))
endif

include $(X_ROOT)/scripts/make-rules/common-versions.mk
...
# Build options
#
PRJ_SRC_PATH :=github.com/fleezesd/xnightwatch
...
CERTIFICATES=x-apiserver admin
endif

MANIFESTS_DIR=$(X_ROOT)/manifests
SCRIPTS_DIR=$(X_ROOT)/scripts

# Image build releated variables.
# REGISTRY_PREFIX ?= ccr.ccs.tencentyun.com/superproj
GENERATED_DOCKERFILE_DIR=$(X_ROOT)/build/docker

# Kubernetes releated variables.
## Metadata for driving the build lives here.
META_DIR := $(X_ROOT)/.make
...
APIROOT ?= $(X_ROOT)/pkg/api
APISROOT ?= $(X_ROOT)/pkg/apis
