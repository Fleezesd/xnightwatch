...
X_MODULE_NAME="github.com/fleezesd/xnightwatch"
...
OUTPUT_PKG="github.com/fleezesd/xnightwatch/pkg/generated"
...
if [[ "${DBG_CODEGEN}" == 1 ]]; then
    x::log::status "DBG: starting generated_files"
fi

# Generate a list of directories we don't want to play in.
DIRS_TO_AVOID=()
x::util::read-array DIRS_TO_AVOID < <(
...
if [[ "${DBG_CODEGEN}" == 1 ]]; then
    x::log::status "DBG: finding all +k8s: tags"
fi
ALL_K8S_TAG_FILES=()
x::util::read-array ALL_K8S_TAG_FILES < <(
    git_grep -l \
...
if [[ "${DBG_CODEGEN}" == 1 ]]; then
    x::log::status "DBG: found ${#ALL_K8S_TAG_FILES[@]} +k8s: tagged files"
fi
...
function codegen::protobuf() {
    ...
    # source tree.  This is managed in x::build::copy_output in build/common.sh.
    # If the output set is changed update that function.

    local apis=()
    x::util::read-array apis < <(
        git grep --untracked --null -l \
            -e '// +k8s:protobuf-gen=package' \
            -- \
            cmd pkg \
            | while read -r -d $'\0' F; do dirname "${F}"; done \
            | sed 's|^|github.com/superproj/xnightwatch/|;s|k8s.io/kubernetes/staging/src/||' \
            | sort -u)

    x::log::status "Generating protobufs for ${#apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: generating protobufs for:"
        for dir in "${apis[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
    ...
    # X-apiserver: undefined: ObjectReference.
    ...
    if x::protoc::check_protoc >/dev/null; then
    ...
    # cp ${X_ROOT}/manifests/generated.pb.go.fix ${X_ROOT}/pkg/apis/apps/v1beta1/generated.pb.go
}
...
function codegen::deepcopy() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: finding all +k8s:deepcopy-gen tags"
    fi
    local tag_dirs=()
    x::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:deepcopy-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            | while read -r -d $'\0' F; do dirname "${F}"; done \
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: found ${#tag_dirs[@]} +k8s:deepcopy-gen tagged dirs"
    fi
    ...
    x::log::status "Generating deepcopy code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running deepcopy-gen for:"
        for dir in "${tag_dirs[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
    ...
    deepcopy-gen \
        ...
        --bounding-dirs "${X_MODULE_NAME},k8s.io/api" \
        "${tag_pkgs[@]}" \
        "$@"

    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated deepcopy code"
    fi
}
...
function gen_types_swagger_doc() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running gen-swaggertype-docs for ${group_version} at ${gv_dir}"
    fi
...
function codegen::swagger() {
    ...
    if ! command -v gen-swaggertype-docs &> /dev/null ; then
        GOPROXY=off go install ${X_ROOT}/cmd/gen-swaggertype-docs
    fi

    local group_versions=()
    IFS=" " read -r -a group_versions <<< "${X_AVAILABLE_GROUP_VERSIONS}"

    x::log::status "Generating swagger for ${#group_versions[@]} targets"

    git_find -z ':(glob)**/types_swagger_doc_generated.go' | xargs -0 rm -f

    # Regenerate files.
    for group_version in "${group_versions[@]}"; do
      gen_types_swagger_doc "${group_version}" "$(x::util::group-version-to-pkg-path "${group_version}")"
    done
}
...
function codegen::prerelease() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: finding all +k8s:prerelease-lifecycle-gen tags"
    fi
    local tag_dirs=()
    x::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:prerelease-lifecycle-gen=true' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: found ${#tag_dirs[@]} +k8s:prerelease-lifecycle-gen tagged dirs"
    fi
    ...
    x::log::status "Generating prerelease-lifecycle code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running prerelease-lifecycle-gen for:"
        for dir in "${tag_dirs[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated prerelease-lifecycle code"
    fi
}
...
function codegen::defaults() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: finding all +k8s:defaulter-gen tags"
    fi
    local tag_dirs=()
    x::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:defaulter-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: found ${#tag_dirs[@]} +k8s:defaulter-gen tagged dirs"
    fi
    ...
    x::log::status "Generating defaulter code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running defaulter-gen for:"
        for dir in "${tag_dirs[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated defaulter code"
    fi
}
...
function codegen::conversions() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: finding all +k8s:conversion-gen tags"
    fi
    local tag_dirs=()
    x::util::read-array tag_dirs < <(\
        grep -l --null '^// *+k8s:conversion-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: found ${#tag_dirs[@]} +k8s:conversion-gen tagged dirs"
    fi
    ...
    x::log::status "Generating conversion code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running conversion-gen for:"
        for dir in "${tag_dirs[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated conversion code"
    fi
}
...
function todo::codegen::openapi() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: finding all +k8s:openapi-gen tags"
    fi

    local tag_files=()
    x::util::read-array tag_files < <(
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: found ${#tag_dirs[@]} +k8s:openapi-gen tagged dirs"
    fi
    ...
    x::log::status "Generating openapi code"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running openapi-gen for:"
        for dir in "${tag_dirs[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated openapi code"
    fi
}
...
function codegen::openapi() {
    ...
    local output_dir="pkg/generated/openapi"
    local output_pkg="github.com/fleezesd/xnightwatch/${output_dir}"
    local known_violations_file="${API_KNOWN_VIOLATIONS_DIR}/violation_exceptions.list"
...
function codegen::applyconfigs() {
    ...
    local ext_apis=()
    x::util::read-array ext_apis < <(
        cd "${X_ROOT}"
        git_find -z ':(glob)pkg/apis/**/*types.go' \
            | while read -r -d $'\0' F; do dirname "${X_MODULE_NAME}/${F}"; done \
            | sort -u)
    ext_apis+=("${EXTRA_GENERATE_PKG[@]}")
    ext_apis+=("k8s.io/apimachinery/pkg/apis/meta/v1" "k8s.io/api/autoscaling/v1")

    x::log::status "Generating apply-config code for ${#ext_apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running applyconfiguration-gen for:"
        for api in "${ext_apis[@]}"; do
            x::log::status "DBG:     $api"
        done
    fi
    ...
    applyconfiguration-gen \
        ...
        --go-header-file "${BOILERPLATE_FILENAME}" \
        --output-dir "${X_ROOT}/pkg/generated/applyconfigurations" \
        --output-pkg "${APPLYCONFIG_PKG}" \
        "${ext_apis[@]}" \
        "$@"

    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated apply-config code"
    fi
}
...
function codegen::clients() {
    ...
    for gv in "${group_versions[@]}"; do
        ...
        local api_dir
        api_dir=$(x::util::group-version-to-pkg-path "${gv}")
    ...
    x::log::status "Generating client code for ${#gv_dirs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running client-gen for:"
    ...
    client-gen \
        ...
        --output-dir "${X_ROOT}/pkg/generated/clientset" \
    ...
    ${X_ROOT}/scripts/fix-generated-client.sh

    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated client code"
    fi
}
...
function codegen::listers() {
    ...
    x::util::read-array ext_apis < <(
        cd "${X_ROOT}"
        git_find -z ':(glob)pkg/apis/**/*types.go' \
            | while read -r -d $'\0' F; do dirname "github.com/fleezesd/xnightwatch/${F}"; done \
            | sort -u)
    ext_apis+=("${EXTRA_GENERATE_PKG[@]}")

    x::log::status "Generating lister code for ${#ext_apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running lister-gen for:"
        for api in "${ext_apis[@]}"; do
            x::log::status "DBG:     $api"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated lister code"
    fi
}
...
function codegen::informers() {
    ...
    x::log::status "Generating informer code for ${#ext_apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: running informer-gen for:"
        for api in "${ext_apis[@]}"; do
            x::log::status "DBG:     $api"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "Generated informer code"
    fi
}
...
function unused::codegen::subprojects() {
    ...
    local codegen
    codegen="${X_ROOT}/staging/src/k8s.io/code-generator"
    for sub in "${subs[@]}"; do
        x::log::status "Generating code for subproject ${sub}"
        pushd "${sub}" >/dev/null
        CODEGEN_PKG="${codegen}" \
        UPDATE_API_KNOWN_VIOLATIONS="${UPDATE_API_KNOWN_VIOLATIONS}" \
...
function unused::codegen::protobindings() {
    ...
    x::log::status "Generating protobuf bindings for ${#apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        x::log::status "DBG: generating protobuf bindings for:"
        for dir in "${apis[@]}"; do
            x::log::status "DBG:     $dir"
        done
    fi
