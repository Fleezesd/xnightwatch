...
[[ $(type -t x::protoc::loaded) == function ]] && return 0
...
# Generates $1/api.pb.go from the protobuf file $1/api.proto
# $1: Full path to the directory where the api.proto file is
function x::protoc::protoc() {
  local package=${1}
  gogopath=$(dirname "$(x::util::find-binary "protoc-gen-gogo")")
  ...
  (
    ...
    PATH="${gogopath}:${PATH}" protoc \
      --proto_path="$(pwd -P)" \
      --proto_path="${X_ROOT}/vendor" \
      --proto_path="${X_ROOT}/third_party/protobuf" \
  ...
  (
    ...
    if [[ ! -f protoc  ]]; then
      ...
      if [[ ${os} == "darwin" ]]; then
        ...
        x::log::info "This install script does not support ${os}/${arch}"
        return 1
      fi
      x::util::download_file "${url}" "${download_file}"
    ...
    x::log::info "$HOME/bin/protoc v${PROTOC_VERSION} installed."
  )
}
