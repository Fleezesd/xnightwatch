...
function x::util::sourced_variable {
...
function x::util::sortable_date() {
  date "+%Y%m%d-%H%M%S"
}

# arguments: target, item1, item2, item3, ...
# returns 0 if target is in the given items, 1 otherwise.
function x::util::array_contains() {
  local search="$1"
  local element
  shift
  for element; do
...
function x::util::wait_for_url() {
  ...
  command -v curl >/dev/null || {
    x::log::usage "curl must be installed"
    exit 1
  }
  ...
  for i in $(seq 1 "${times}"); do
    ...
    if out=$(curl --max-time "${maxtime}" -gkfs "${url}" 2>/dev/null); then
      x::log::status "On try ${i}, ${prefix}: ${out}"
      return 0
    fi
    sleep "${wait}"
  done
  x::log::error "Timed out waiting for ${prefix} to answer at ${url}; tried ${times} waiting ${wait} between each"
  return 1
}

function x::util::wait_for_url_with_bearer_token() {
  local url=$1
  local token=$2
  local prefix=${3:-}
  local wait=${4:-1}
  ...
  x::util::wait_for_url "${url}" "${prefix}" "${wait}" "${times}" "${maxtime}" -H "Authorization: Bearer ${token}"
}

# Example:  x::util::wait_for_success 120 5 "nodectl get nodes|grep localhost"
# arguments: wait time, sleep time, shell command
# returns 0 if the shell command get output, 1 otherwise.
function x::util::wait_for_success(){
  local wait_time="$1"
  local sleep_time="$2"
  local cmd="$3"
...
# Example:  x::util::trap_add 'echo "in trap DEBUG"' DEBUG
# See: http://stackoverflow.com/questions/3338030/multiple-bash-traps-for-the-same-signal
function x::util::trap_add() {
  local trap_add_cmd
  trap_add_cmd=$1
  shift
...
# Opposite of x::util::ensure-temp-dir()
function x::util::cleanup-temp-dir() {
  rm -rf "${X_TEMP}"
}
...
# Vars set:
#   X_TEMP
function x::util::ensure-temp-dir() {
  if [[ -z ${X_TEMP-} ]]; then
    X_TEMP=$(mktemp -d 2>/dev/null || mktemp -d -t onex.XXXXXX)
    x::util::trap_add x::util::cleanup-temp-dir EXIT
  fi
}

function x::util::host_os() {
  local host_os
  case "$(uname -s)" in
    Darwin)
      host_os=darwin
    ...
    Linux)
      ...
      ;;
    *)
      x::log::error "Unsupported host OS.  Must be Linux or Mac OS X."
      exit 1
      ;;
  esac
  echo "${host_os}"
...
function x::util::host_arch() {
  ...
  case "$(uname -m)" in
    ...
    ppc64le*)
      ...
      ;;
    *)
      x::log::error "Unsupported host arch. Must be x86_64, 386, arm, arm64, s390x or ppc64le."
      exit 1
      ;;
  esac
  echo "${host_arch}"
...
function x::util::host_platform() {
  echo "$(x::util::host_os)/$(x::util::host_arch)"
}

# looks for $1 in well-known output locations for the platform ($2)
# $X_ROOT must be set
function x::util::find-binary-for-platform() {
  local -r lookfor="$1"
  local -r platform="$2"
  local locations=(
    "${X_ROOT}/_output/bin/${lookfor}"
    "${X_ROOT}/_output/dockerized/bin/${platform}/${lookfor}"
    "${X_ROOT}/_output/local/bin/${platform}/${lookfor}"
    "${X_ROOT}/platforms/${platform}/${lookfor}"
  )

  # if we're looking for the host platform, add local non-platform-qualified search paths
  if [[ "${platform}" = "$(x::util::host_platform)" ]]; then
    locations+=(
      "${X_ROOT}/_output/local/go/bin/${lookfor}"
      "${X_ROOT}/_output/dockerized/go/bin/${lookfor}"
    );
  fi
  ...
  if [[ -z "${bin}" ]]; then
    x::log::error "Failed to find binary ${lookfor} for platform ${platform}"
    return 1
  fi
...
# $X_ROOT must be set
function x::util::find-binary() {
  x::util::find-binary-for-platform "$1" "$(x::util::host_platform)"
}
...
function x::util::group-version-to-pkg-path() {
  ...
  case "${group_version}" in
    ...
    __internal)
      ...
      ;;
    #core/v1)
      #echo "${X_ROOT}/pkg/apis/core/v1"
      #;;
    apps/v1beta1)
      echo "${X_ROOT}/pkg/apis/apps/v1beta1"
      ;;
    #coordination/v1)
      #echo "${X_ROOT}/pkg/apis/coordination/v1"
      #;;
    #apiextensions/v1)
      #echo "${X_ROOT}/pkg/apis/apiextensions/v1"
      #;;
    *)
...
function x::util::gv-to-swagger-name() {
  local group_version="$1"
  case "${group_version}" in
    v1)
      echo "v1"
...
function x::util::git_upstream_remote_name() {
...
function x::util::ensure_clean_working_dir() {
...
function x::util::base_ref() {
  ...
  full_branch="$(x::util::git_upstream_remote_name)/${git_branch}"
...
function x::util::has_changes() {
  local -r git_branch=$1
  local -r pattern=$2
  local -r not_pattern=${3:-totallyimpossiblepattern}

  local base_ref
  base_ref=$(x::util::base_ref "${git_branch}")
...
function x::util::test_openssl_installed {
...
function x::util::test_client_certificate_authentication_enabled {
  local output
  x::util::test_openssl_installed
...
function x::util::create_signing_certkey {
    local sudo=$1
    local dest_dir=$2
    local id=$3
    local purpose=$4
...
function x::util::create_client_certkey {
    local sudo=$1
    local dest_dir=$2
    local ca=$3
    local id=$4
...
function x::util::create_serving_certkey {
    local sudo=$1
    local dest_dir=$2
    local ca=$3
    local id=$4
...
function x::util::write_client_nodeconfig {
    local sudo=$1
    local dest_dir=$2
    local ca_file=$3
    local api_host=$4
...
EOF
    ...
    username=$(whoami)
    ${sudo} /usr/bin/env bash -e <<EOF
    $(x::util::find-binary nodectl) --nodeconfig="${dest_dir}/${client_id}.nodeconfig" config view --minify --flatten > "/tmp/${client_id}.nodeconfig"
...
function x::util::ensure_docker_daemon_connectivity {
...
# x::util::join <delim> <list...>
# Concatenates the list elements with the delimiter passed as first parameter
#
# Ex: x::util::join , a b c
#  -> a,b,c
function x::util::join {
  local IFS="$1"
  shift
  echo "$*"
}
...
# x::util::ensure-docker-buildx
# Check if we have "docker buildx" commands available
#
function x::util::ensure-docker-buildx {
...
#
function x::util::ensure-bash-version {
...
# x::util::ensure-gnu-sed
# Determines which sed binary is gnu-sed on linux/darwin
#
# Sets:
#  SED: The name of the gnu-sed binary
#
function x::util::ensure-gnu-sed {
  ...
  if echo "${sed_help}" | grep -q "GNU\|BusyBox"; then
    ...
    x::log::error "Failed to find GNU sed as sed or gsed. If you are on Mac: brew install gnu-sed." >&2
    return 1
  fi
  x::util::sourced_variable "${SED}"
}

# x::util::ensure-gnu-date
# Determines which date binary is gnu-date on linux/darwin
#
# Sets:
#  DATE: The name of the gnu-date binary
#
function x::util::ensure-gnu-date {
  ...
  if echo "${date_help}" | grep -q "GNU\|BusyBox"; then
    ...
    x::log::error "Failed to find GNU date as date or gdate. If you are on Mac: brew install coreutils." >&2
    return 1
  fi
  x::util::sourced_variable "${DATE}"
}

# x::util::check-file-in-alphabetical-order <file>
# Check that the file is in alphabetical order
#
function x::util::check-file-in-alphabetical-order {
...
function x::util::require-jq {
  if ! command -v jq &>/dev/null; then
    x::log::error  "jq not found. Please install."
    return 1
  fi
}
...
#  $ x::util::read-array W < <(echo -ne "1 1\n2 2\n3 3\n")
#  bash: W is defined but isn't an array
#  $ x::util::read-array X < <(echo -ne "1 1\n2 2\n3 3\n")
#  $ x::util::read-array Y < <(echo -ne "1 1\n2 2\n3 3\n")
#  bash: Y is defined but isn't an array
#  $ x::util::read-array Z < <(echo -ne "1 1\n2 2\n3 3\n")
#  $ declare -p W X Y Z
#  declare -- W="abc"
#  declare -a X=([0]="1 1" [1]="2 2" [2]="3 3")
#  declare -A Y
#  declare -a Z=([0]="1 1" [1]="2 2" [2]="3 3")
function x::util::read-array {
  if [[ -z "$1" ]]; then
...
if [[ -z "${color_start-}" ]]; then
  ...
  x::util::sourced_variable "${color_start}"
  x::util::sourced_variable "${color_red}"
  x::util::sourced_variable "${color_yellow}"
  x::util::sourced_variable "${color_green}"
  x::util::sourced_variable "${color_blue}"
  x::util::sourced_variable "${color_cyan}"
  x::util::sourced_variable "${color_norm}"
fi

function x::util::read-available-cpus {
  if [ -z "${max_cpus}" ]; then
    max_cpus=1
