...
[[ $(type -t x::verify::generated::loaded) == function ]] && return 0
...
# in the work tree.
#
# Example: x::verify::generated "Mock files are out of date" "Please run 'scripts/update-mocks.sh'" scripts/update-mocks.sh
x::verify::generated() {
  ( # a subshell prevents environment changes from leaking out of this function
    ...
    x::util::ensure_clean_working_dir

    # This sets up the environment, like GOCACHE, which keeps the worktree cleaner.
    x::golang::setup_env

    _tmpdir="$(x::realpath "$(mktemp -d -t "verify-generated-$(basename "$1").XXXXXX")")"
    git worktree add -f -q "${_tmpdir}" HEAD
    x::util::trap_add "git worktree remove -f ${_tmpdir}" EXIT
    cd "${_tmpdir}"
